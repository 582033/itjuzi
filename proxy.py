...
class proxy():
    ...
    def get(self):
        ...
        for proxy in proxies:
            t = threading.Thread(target=self._check_proxy, args=(proxy, queue))
            threads.append(t)
            t.start()
    ...
    def _check_proxy(self, proxy, queue):
        #如果代理有效则加入队列
        try:
            html = requests.get(self.target_url, headers=self.headers, proxies={'http':'http://%s' % proxy}, timeout=self.timeout)
            #print html.text
            h = re.findall(r'%s' % ver_keyword, html.text)
            tip = "未"
            if len(h) > 0:
                queue.put(proxy)
                tip = "已"
            #print "[%s]匹配到关键字: %s 个, %s加入队列" % (proxy, len(h), tip)
        except Exception:
            pass
            #continue
